...
#CRG - small batch size...may need to make this larger
...
def ResidualBlock(name, input_dim, output_dim, filter_size, inputs, resample=None, he_init=True):
    ...
    #Normal 2D Conv
        ...
        conv_shortcut = lib.ops.conv2d.Conv2D
	#CRG functools is a python library and functools.partial calls the conv2d library with input and output dimensions
        conv_1        = functools.partial(lib.ops.conv2d.Conv2D, input_dim=input_dim,  output_dim=input_dim)
...
def DCGANGenerator(n_samples, noise=None, dim=DIM, bn=True, nonlinearity=tf.nn.relu):
    ...
    #CRG start with 128 dimensions
    if noise is None:
        noise = tf.random_normal([n_samples, 128])

    #CRG fully connected layer to start
    output = lib.ops.linear.Linear('Generator.Input', 128, 4*4*8*dim, noise)
    output = tf.reshape(output, [-1, 8*dim, 4, 4])
    if bn:
        #CRG Nomarlize is batchnorm, [0,2,3] is axes??
        output = Normalize('Generator.BN1', [0,2,3], output)
    #CRG nonlinearity here is relu (see argument in definition)
    output = nonlinearity(output)
    
    #CRG Conv Layer 1, kernel = 5
    ...
    #CRG Conv Layer 2 (same as above)
    ...
    #CRG Conv Layer 3 (same as above)
    ...
    #use tanh instead of sigmoid on output
    output = tf.tanh(output)

    #unset the stdev for the weights globally
...
def DCGANDiscriminator(inputs, dim=DIM, bn=True, nonlinearity=LeakyReLU):
    ...
    #CRG Conv 1 with leakyrelu
    output = lib.ops.conv2d.Conv2D('Discriminator.1', 3, dim, 5, output, stride=2)
    output = nonlinearity(output)

    #CRG Conv 2 with leakyrelu and batchnorm
    output = lib.ops.conv2d.Conv2D('Discriminator.2', dim, 2*dim, 5, output, stride=2)
    if bn:
        output = Normalize('Discriminator.BN2', [0,2,3], output)
    output = nonlinearity(output)

    #CRG Conv 3 with leakyrelu and batchnorm
    output = lib.ops.conv2d.Conv2D('Discriminator.3', 2*dim, 4*dim, 5, output, stride=2)
    if bn:
        output = Normalize('Discriminator.BN3', [0,2,3], output)
    output = nonlinearity(output)

    #CRG Conv 4 with leakyrelu and batchnorm
