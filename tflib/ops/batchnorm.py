...
def Batchnorm(name, axes, inputs, is_training=True, stats_iter=None, update_moving_stats=True, fused=True):
    if ((axes == [0,2,3]) or (axes == [0,2])) and fused==True:
        ...
        #CRG if is_training is None:
        if is_training is True:
            outputs, batch_mean, batch_var = _fused_batch_norm_training()
        else:
            outputs, batch_mean, batch_var = _fused_batch_norm_inference()
            # tf.cond returns _fused_batch_norm_training if is_training is true and _fused_batch_norm_inference if is_training is false.
